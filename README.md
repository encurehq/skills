...
| `add-issues`            | Any stack + Linear       | Draft and publish one or more Linear issues, each a phased execution plan another agent can pick up cold and run — publishes directly, no confirmation step; falls back to chat/local files if the Linear MCP isn't connected.                                                                      |
